...
(() => {
  ...
  const buildMailto = (subject, body) => {
    ...
    // würde Leerzeichen als "+" kodieren, was einige Mailprogramme wörtlich
    // anzeigen. Deshalb bauen wir die Mailto-Parameter hier bewusst selbst.
    const params = [];
  ...
  */
  function renderCatalog() {
    ...
    // Standardansicht 'Alle': bewusst Kategorien mischen, damit oben direkt
    // unterschiedliche Produktarten sichtbar sind.
    const interleaveDefaultAll = sourceItems => {
      const order = ['perlen', 'straenge', 'rohsteine', 'geschliffen', 'beads'];
      const buckets = new Map(order.map(key => [key, []]));
      const extra = [];
      sourceItems.forEach(item => {
        if (buckets.has(item.category)) buckets.get(item.category).push(item);
    ...
    if (sort === 'title') {
      ...
      items = interleaveDefaultAll(items);
    }
